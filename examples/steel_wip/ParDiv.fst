 ...
 *)
noeq
type action (#s:Type u#1) (c:comm_monoid s) (a:Type u#a) : Type u#(max a 1) = {
   pre: c.r;
   post: a -> c.r;
   sem: (frame:c.r ->
...
let rec step #s #c (i:nat) #pre #a #post (f:m s c a pre post) (frame:c.r) (state:s)
  ...
  = match f with
    ...
    | Par pre0 post0 m0
        ...
        else ( // Extra assertions are required to trigger rewriting rules
               assert (((pre0 `c.star` pre1) `c.star` frame) `c.equals` ((pre1 `c.star` pre0) `c.star` frame));
